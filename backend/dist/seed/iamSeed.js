...
exports.ensureIamSeeded = ensureIamSeeded;
...
async function ensureIamSeeded() {
    // Permissions
    ...
    for (const role of ROLES) {
        const r = await prisma_1.prisma.role.upsert({
            where: { key: role.key },
        ...
        if (role.isSystem) {
            const perms = await prisma_1.prisma.permission.findMany({ where: { key: { in: role.permissionKeys } } });
            const desired = new Set(perms.map((p) => p.id));
            const existing = await prisma_1.prisma.rolePermission.findMany({ where: { roleId: r.id } });
            const existingSet = new Set(existing.map((rp) => rp.permissionId));
            for (const rp of existing) {
                if (!desired.has(rp.permissionId))
                    await prisma_1.prisma.rolePermission.delete({ where: { roleId_permissionId: { roleId: rp.roleId, permissionId: rp.permissionId } } });
            }
            for (const permId of desired) {
                if (!existingSet.has(permId))
                    await prisma_1.prisma.rolePermission.create({ data: { roleId: r.id, permissionId: permId } });
            }
        }
    }
    // Ensure each user has a matching UserRole entry for their primary role.
    const rolesByKey = await prisma_1.prisma.role.findMany();
    const roleIdByKey = new Map(rolesByKey.map((r) => [r.key, r.id]));
