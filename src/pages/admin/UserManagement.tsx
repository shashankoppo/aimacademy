...
const UserManagement = () => {
  ...
  const openCreate = () => {
    setForm({ name: "", email: "", phone: "", role: "STUDENT", password: "", isActive: true });
    setCreateOpen(true);
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-500 font-medium">Create accounts, assign roles, reset passwords, and activate/deactivate users.</p>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-white border-b border-slate-100">
                ...
                <th className="p-4 text-[11px] font-bold uppercase tracking-widest text-slate-400">Role</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {filtered.map((u) => (
                <tr key={u.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="p-4 text-sm text-slate-500 font-body">{u.role}</td>
        ...
        <DialogContent className="sm:max-w-[520px] max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid gap-4 py-2">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2">
                <Label>Role</Label>
                <Select value={form.role} onValueChange={(v) => setForm((p) => ({ ...p, role: v as RoleKey }))}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
